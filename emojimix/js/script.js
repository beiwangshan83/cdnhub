...
const advertise = () => {
	...
	//if(counter == 10) {
	//	if(confirm("I hope that you are enjoying emojimix!\n\nI have a lot of other projects on my website and I would love if you could check them out. Would that be ok?")) {
...
const triggerDownload = event => {
	...
	if(confirm("Download image instead of copying?")) downloadImage();
}
...
const copyImage = async () => {
	...
	if(!navigator.clipboard || typeof ClipboardItem == "undefined") {
		showMessage("Your browser does not support copying images. Downloading instead...");
	...
	try {
		...
		if(navigator.userAgent.includes("Instagram")) showMessage("Open emojimix in your browser to copy");
		else {
			showMessage("Error copying image. Downloading instead...");
			return setTimeout(downloadImage, 1000);
		}
	}
};
...
const copyLink = async () => {
	if(!navigator.clipboard) {
		if(navigator.userAgent.includes("Instagram")) showMessage("Open emojimix in your browser to share");
		else showMessage("Your browser does not support copying");
		return;
	}
	try {
		...
		if(navigator.userAgent.includes("Instagram")) showMessage("Open emojimix in your browser to copy");
		else showMessage("Error copying link");
	}
};

const shareImage = async () => {
	if(!navigator.share) return showMessage("Your browser does not support sharing");
	try {
		showMessage("Sharing...");
		let shareData = {
			title: "emojimix",
...
const downloadEmojis = async () => {
	try {
		...
		loadSection.text.innerText = `Click here to ${urlEmojis ? "continue" : "begin"}`;
